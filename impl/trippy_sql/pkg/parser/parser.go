...
// Parse takes a string representing a SQL query and parses it into a AST struct. It may fail.
func Parse(sqls string) (AST, error) {
...
// ParseMany takes a string slice representing many SQL queries and parses them into a AST struct slice.
// It may fail. If it fails, it will stop at the first failure.
func ParseMany(sqls []string) ([]AST, error) {
	qs := []AST{}
...
func parse(sql string) (AST, error) {
	return (&parser{0, strings.TrimSpace(sql), stepType, AST{}, nil, ""}).parse()
}
...
func (p *parser) doParse() (AST, error) {
	for {
		if p.i >= len(p.sql) {
